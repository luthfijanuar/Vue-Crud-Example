<template>
  <ul class="list-group">
    <li class="list-group-item">
      Subtotal ({{itemsQuantity}} {{'item' | pluralize(itemsQuantity) }}): {{subtotal | currency}}
    ...
    <li class="list-group-item">Taxes: {{taxes | currency}}</li>

    <li class="list-group-item">
      <strong>Total:</strong>
      <strong v-if="!totalDiscount"> {{total | currency}}</strong>
      <span v-if="totalDiscount">
        <strike> {{total + 3000 | currency}}</strike>
